...
import { getSingleFieldHistory } from '../server/clients/manageRecallsApi/manageRecallsApiClient'
import * as configModule from '../server/config'
import getFieldHistoryResponseJson from '../fake-manage-recalls-api/stubs/__files/get-single-field-history.json'
...
pactWith({ consumer: 'manage-recalls-ui', provider: 'manage-recalls-api' }, provider => {
  ...
  describe('get uploaded recall field history', () => {
    test("can successfully retrieve a field's history", async () => {
      ...
      const actual = await getSingleFieldHistory(recallId, fieldId, token)
    ...
    test('returns 401 if invalid user', async () => {
      ...
      try {
        await getSingleFieldHistory(recallId, fieldId, token)
      } catch (exception) {
