...
import { updateRescindRecord } from '../server/clients/manageRecallsApiClient'
...
pactWith({ consumer: 'manage-recalls-ui', provider: 'manage-recalls-api' }, provider => {
  ...
  const rescindRecordId = '33333333-0000-0000-0000-000000000000'

  const emailSentDate = '2021-10-05'
  ...
  const path = `/recalls/${recallId}/rescind-records/${rescindRecordId}`
  ...
  describe('update a rescind record', () => {
    test('can successfully add a record', async () => {
      await provider.addInteraction({
        ...
        ...pactPatchRequest(
          'an update rescind record request',
          path,
          {
            details,
      ...
      const actual = await updateRescindRecord({ recallId, valuesToSave, user })
    ...
    test('returns 401 if invalid user', async () => {
      await provider.addInteraction({
        ...
        ...pactPatchRequest(
          'an unauthorized update record request',
          path,
          {
            details,
            emailFileContent: valuesToSave.fileContent,
      ...
      try {
        await updateRescindRecord({ recallId, valuesToSave, user })
      } catch (exception) {
