...
import { addRescindRecord } from '../server/clients/manageRecallsApiClient'
...
pactWith({ consumer: 'manage-recalls-ui', provider: 'manage-recalls-api' }, provider => {
  ...
  const emailReceivedDate = '2021-10-05'
  ...
  describe('add a record of a rescind request', () => {
    test('can successfully add a record', async () => {
      ...
      const actual = await addRescindRecord({ recallId, valuesToSave, user })
    ...
    test('returns 401 if invalid user', async () => {
      ...
      try {
        await addRescindRecord({ recallId, valuesToSave, user })
      } catch (exception) {
