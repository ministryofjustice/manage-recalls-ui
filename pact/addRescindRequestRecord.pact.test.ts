...
import { addRescindRequestRecord } from '../server/clients/manageRecallsApiClient'
  ...
  describe('add a record of a rescind request', () => {
    test('can successfully add a record', async () => {
      await provider.addInteraction({
        ...
        ...pactPostRequest('an add rescind record request', path, requestBody, accessToken),
        willRespondWith: { status: 201 },
      })

      const actual = await addRescindRequestRecord(recallId, requestBody, accessToken)
    ...
    test('returns 401 if invalid user', async () => {
      ...
      try {
        await addRescindRequestRecord(recallId, requestBody, accessToken)
      } catch (exception) {
