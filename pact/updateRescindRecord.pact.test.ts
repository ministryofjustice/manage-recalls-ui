...
pactWith({ consumer: 'manage-recalls-ui', provider: 'manage-recalls-api' }, provider => {
  ...
  describe('update a rescind record', () => {
    test('can successfully add a record', async () => {
      ...
      const actual = await updateRescindRecord(recallId, rescindRecordId, requestBody, accessToken)
    ...
    test('returns 401 if invalid user', async () => {
      await provider.addInteraction({
        ...
        ...pactPatchRequest('an unauthorized update record request', path, requestBody, accessToken),
        willRespondWith: { status: 401 },
      })

      try {
        await updateRescindRecord(recallId, rescindRecordId, requestBody, accessToken)
      } catch (exception) {
